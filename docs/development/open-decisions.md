...
| x402 signer boundary | SettleDiff stores no wallet key. A separately installed signer owns authority and is invoked once through the versioned signer contract (request schema 2, result and metadata schema 3) with a controlled environment. The signer reconstructs schema-2 `PaymentTerms`, including any advertised response-contract digest, before signing. | External signer contract and persistence scans; 2026-09-22 assurance validation |
