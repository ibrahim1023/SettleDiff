...
| x402 signer boundary | SettleDiff stores no wallet key. A separately installed signer owns authority and is invoked once through the versioned signer contract (request schema 2, result and metadata schema 3) with a controlled environment. | External signer contract and persistence scans |
