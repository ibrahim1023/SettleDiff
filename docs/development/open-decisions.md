...
Context.dev is required for live investigations; this owner decision was resolved during Task 12. No input is required for PydanticAI, Hyperfusion as the provider, the single-agent boundary, deterministic verdict ownership, fixture-first tests, optional OpenTelemetry, SQLite, or the server-rendered UI; those decisions are accepted in the existing ADRs.
