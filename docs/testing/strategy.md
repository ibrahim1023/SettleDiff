...
`fixtures/failed-broadcast/`). Raw live evidence bundles remain local and untracked. The first controlled x402 Base Sepolia cycle is recorded in [x402-live-cycle](x402-live-cycle.md); its provider-amount omission and unrelated receipt log were reduced to offline regressions.
